    ...
    <div class="wrapper">
        <nav class="main-header navbar navbar-expand {{setting('nav_color','navbar-light navbar-white')}} border-bottom-0" style="background-color: yellow">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="{!! route('appointment_top') !!}" style="color: #837b82; font-weight: bold"><p>{{trans('lang.appointment_plural')}}</p></a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="{!! route('address_top') !!}" style="color: #837b82; font-weight: bold"><p>{{trans('lang.address_plural')}}</p></a>
                </li>
            </ul>
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link {{ Request::is('favorites*') ? 'active' : '' }}"
                        href="{{route('favorites.index')}}"><i class="fas fa-heart" style="color: #ada1ac"></i></a>
                </li>
                @endcan -->
                @can('notifications.index')
                <li class="nav-item">
                    <a class="nav-link {{ Request::is('notifications*') ? 'active' : '' }}"
                        href="{!! route('notifications.index') !!}"><i class="fas fa-bell"
                            style="color: #ada1ac"></i></a>
                </li>
                @endcan
                <!-- <li class="nav-item dropdown">
                    <a class="nav-link" data-toggle="dropdown" href="#"> <i class="fa fas fa-angle-down"
                            style="color: #ada1ac"></i> {!! Str::upper(app()->getLocale()) !!}
                    </a>
                    <div class="dropdown-menu dropdown-menu-right">
                        {!! Form::open(['url' => ['settings/updateLanguage'], 'method' =>
                        'patch','id'=>'languages-form']) !!}
                        {!! Form::hidden('locale',app()->getLocale(),['id'=>'current-language'])!!}
                        @foreach(getAvailableLanguages() as $locale => $lang)
                        <a href="#" style="color: #ada1ac"
